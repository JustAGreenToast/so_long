...
/*   Updated: 2024/01/22 13:09:29 by pfontenl         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	move_player(int keycode, t_data *data)
{
	...
	if (!tile_stepped(data->map_data.map[new_pos], new_pos, data))
		return (0);
	data->map_data.revealed[new_pos] = 1;
	data->player_data.pos = new_pos;
	data->player_data.dir = new_dir;
...
static int	tile_stepped(char tile, int pos, t_data *data)
{
	if (tile == TILE_WALL)
		return (0);
	if (tile == TILE_COIN && data->map_data.revealed[pos])
	{
		data->map_data.map[pos] = TILE_EMPTY;
		data->map_data.coins--;
		ft_printf("Cherries left: %d\n", (int)data->map_data.coins);
	}
	if (tile == TILE_SPIKE || tile == TILE_MIMIC)
		game_exit(EXIT_DEFEAT);
